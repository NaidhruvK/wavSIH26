...
Drop **`zoo\corpus\rf\16qam_13dB_2021.wav`**. Expected (~14 s on the card; person 1
measured **17.2 s**, so a wait that long is not a hang):
...
| 14 Sep | **person 1 — nehal**, this laptop, browser click (refusal line said aloud: not confirmed in the report) | **12.62 s**, repeat upload, 7/7 ok (`run_d136143f672e` screens): S3 QPSK, S4 block 8×12 period 96 0o171/0o133, S6 99.9 % printable, sentence visible. Rate 1/2 and K=7 are not shown on the S4 card | **14.50 s**, S0–S3 ok, S4 LOW CONF (period-only), S5 and S6 FAILED (`run_39a874954c5d` screens) | **20.77 s**, TEXT RECOVERED | not run (CLI is the backup) |
| 14 Sep | **person 2** — fresh clone on DOMAINOFSAHITH (a second Windows 11 machine, D:\Dev\SIH, no reused tree), clone at 783be37 (contains 4ea64d9). `/analyze`, **not a human**: UI was served and verified (`GET /` returned the UI HTML, all three `dist/assets` returned 200) but **not clicked**. py 3.11.9 venv + pip 119 s, `npm install` + `npm run build` 31 s, :8000 healthy in 6.1 s, warm-up 8.9 s | **8.8 s**, repeat upload, 7/7 ok (`run_e5c39e8983ff`): fs 200000 `wav_header`, S2 49999.98 Hz at 4.0 sps, S3 QPSK, S4 block 8×12 period 96 rate 1/2 K=7 0o171/0o133, S6 99.9 % printable, sentence present. Rate 1/2 and K=7 read from the report JSON, not an S4 card — PASS | **11.8 s**, S0–S3 ok, S4 LOW CONF (period 96 measured, 284 candidates across 4 families, no permutation inverted), S5 FAILED (conv, reed-solomon, ldpc all declined), S6 FAILED (no payload to frame) (`run_0ee8504c4504`); refusal line written to a file before the upload and it matched — PASS | **9.7 s**, TEXT RECOVERED — PASS | not run (CLI is the backup) |
