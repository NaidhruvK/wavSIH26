...
def run() -> None:
    ...
    lines = [
        ...
        "was considered and rejected too: at the time "
        "pipeline.s1_detect.estimate_snr was itself off by 8-23dB "
        "specifically for 4fsk, so adaptively thresholding the exact "
        "class most affected on a broken SNR estimate would have "
        "traded one gap for a worse one, one day before CORE LOCK. "
        "FIXED 10 Sep -- the estimator now takes a constant-modulus "
        "M2M4 moment estimate and measures within 0.02dB on 4fsk, so "
        "the reason this option was rejected no longer holds and it "
        "is worth re-examining in the robustness window. Left as a known, "
