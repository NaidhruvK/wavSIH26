...
**The three cases the plan named, re-measured on the current build, unmoved —
and two of the numbers I was working from were stale.** Nothing in these fixes
touches a demodulator, so a before/after showing no movement here is the honest
result; the point of re-measuring was to state where it stands rather than to
repeat 5 Sep.
